// 純粋な会派名マッチングロジック。外部依存・server-only を持たないため、
// Next.js の Server だけでなく取り込みスクリプト（tsx）からも再利用できる。
// server-only を必要とする参照は faction-matching.ts 経由で維持する。
 ...
 * 部分一致を許可すると「大田区議会議員団」が「自由民主党大田区議会議員団」など
